    ...
    implements Mage_Shipping_Model_Carrier_Interface
{
	...
	// added on 10th sept ============================================================
	//public $_allowedshippingmethods =  $this->getConfigData('allowed_methods');
    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request)
    {
        if (!$this->getConfigFlag($this->_activeFlag)) {
			...
			foreach ($request->getAllItems() as $key=>$item) {
					...
					// Modified on 8th May 2015
					// If Overweight products are in cart then, Make them free shipping 
					// and shipping method is only "UPS Ground"
					$maxAllowedWeight   = '150';
					
					if($item->getWeight() > $maxAllowedWeight){
						/*echo "hello";
						echo "pkg weight::".$request->getPackageWeight();
						echo "item weight ::".$item->getWeight();
						echo "total";*/
					    ...
					    $this->setAllowedShippingMethods('03');
					}	
		     }
       ...
       //	$this->setRequest($request);
		...
		//=======================================
			...
			foreach($responseobj->getAllRates() as $key=>$rate) {
				...
				if($attribute_flag==1){
					...
					if($freeshippingflag==1){
							foreach($this->_resultFreeShipping->getAllRates() as $rate_free) {
								if(($rate_free->getMethod()==$rate->getMethod()) && $rate->getMethod() == 03){
...
//added on 12th September 2014
 if ($request->getUpsDestType()) {
        ...
        } // set destionation type using billing info on one page
        if(isset($_POST['shipping']['testtype'])){
        	$destType = $_POST['shipping']['testtype'];
        }//set destionation type using shipping info on one page
...
//=========================================================================
    ...
    protected function _parseCgiResponse($response)
    {
        ...
        if (strlen(trim($response))>0) {
            ...
            $allowedMethods = explode(",", $this->getAllowedShippingMethods()); // added on 10th sept
