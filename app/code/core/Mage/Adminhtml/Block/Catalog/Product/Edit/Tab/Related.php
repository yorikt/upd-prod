...
class Mage_Adminhtml_Block_Catalog_Product_Edit_Tab_Related extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function _prepareColumns()
    {
        ...
        //ADDED by jems
        $this->addColumn('prodkey', array(
        		'header'            => Mage::helper('catalog')->__('Prodkey'),
        		'name'              => 'prodkey',
        		'type'              => 'number',
        		'validate_class'    => 'validate-number',
        		'index'             => 'prodkey',
    ...
    {
        /*
         * This code is orignal code comment by jems
         * $products = array();
    	...
    	/**
    	 * start selectd postion and prod keyt code by jems
    	 */
    	...
    	foreach (Mage::registry('current_product')->getRelatedProducts() as $product) {
    		...
    		$select = $connection->select()->from($tableName, array('*'))->where('product_id=?',$this->_getProduct()->getId())
    				  ->where('related_product_id=?',$product->getId());
    	
    		$rowArray =$connection->fetchRow($select);   //return row
    		if($rowArray['related_product_id'] == $product->getId()){
    			$products[$product->getId()] = array('position' => $product->getPosition(),'prodkey'=>$rowArray['product_key']);
    		}else{
    			$products[$product->getId()] = array('position' => $product->getPosition(),'prodkey'=>'0');
    		}
    	}
    	return $products;
    	
    	/**
    	 * end  selectd postion and prod keyt code by jems
    	 */
    }
}