    ...
    implements Mage_Shipping_Model_Carrier_Interface
{
    ...
    public function setAllowedShippingMethods($allowedmethods){
	
		if($allowedmethods!=''){
			$this->_allowedshippingmethods = $allowedmethods; 
		}
	...
	public function getAllowedShippingMethods(){
		...
		if($this->_allowedshippingmethods !=''){		
    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request)
    {
        ...
        //###############
      /*  	echo $total_package_weight = $request->getPackageWeight();
        		echo "@@=====@";
        ...
        $total_package_weight_to_reset_request = $request->getPackageWeight();
        ...
        // Initilize variables
			...
			$freeshippingflag = 0;
			$attribute_flag = 1;
			$specificrateflag = 0; // added on 10th sept
			
			foreach ($request->getAllItems() as $key=>$item) {
			
					$_prod_id = $item->getProductId();
					$product = Mage::getModel('catalog/product')->load($_prod_id);
					
					// get the attributes of a product
					$shipping_rate = $product->getResource()->getAttribute('customshippingrate')->getFrontend()->getValue($product);
			      	$isProductSpecificRate = $product->getResource()->getAttribute('productspecificrate')->getFrontend()->getValue($product);
					$isfreeshipping = $product->getResource()->getAttribute('freeshipping')->getFrontend()->getValue($product);
					
					$total_package_weight = $request->getPackageWeight();
					
					if($isfreeshipping=='No' && $isProductSpecificRate=='No'){
						$attribute_flag = 0;
					}
					
					// added on 10th sept ==============
					if($isProductSpecificRate=='Yes'){
						$prodspecificrateflag = 1;
					}
					//==================================
					...
					if($isfreeshipping=='Yes'){
						$freeshippingflag = 1;
						$total_free_weight += $item->getWeight();
					}
		
					//if isProductSpecificRate is set to Yes and, then exclude item weight
					if($isProductSpecificRate == 'Yes')
					{	
					//echo "package weight = ".$request->getPackageWeight();
					//echo "=====ityem weight = ".$item->getWeight();
					//echo "=====total packecge weiht=". $total_package_weight;
					 
					 
					   $total_package_weight = $request->getPackageWeight() - $item->getWeight();
					   
					   //echo "=====total packecge weiht2=". $total_package_weight;
					   
					   $request->setPackageWeight($total_package_weight);
					   $total_shipping_rate += $shipping_rate;     
							   
					}	
		     }
	     //echo $attribute_flag."==".$freeshippingflag."adfads"; exit;
  		//2nd call to request object
		if($attribute_flag==0 && $freeshippingflag==1){
			$request->setPackageWeight($total_free_weight);
		        $this->setRequest($request);
			$this->_resultFreeShipping = $this->_getQuotes();
		}
		
		if($total_package_weight == 0)
		{	
			$total_package_weight = 0.1;
		}
		$request->setPackageWeight($total_package_weight);

//echo "=====package weight=".$total_package_weight; 
		//===========================================================================

		// added on 10th sept ======================
 	 	if($prodspecificrateflag==1){
			$allowed_methods = "FEDEX_GROUND,GROUND_HOME_DELIVERY";
 	 		$this->setAllowedShippingMethods($allowed_methods);
 	 	}
 	 	//===========================================
		...
		//=======================================
			...
			$responseobj = $this->_result;
			//print_R($responseobj); exit;
			
			foreach($responseobj->getAllRates() as $rate) {
				
				if($attribute_flag==1){
					if($rate->getMethod() == 'FEDEX_GROUND' || $rate->getMethod() == 'GROUND_HOME_DELIVERY'){  
							$rate->setPrice($total_shipping_rate);
					}else{		
							if($request->getPackageWeight()==0){
								$rate->setPrice($total_shipping_rate);
							}else{
								
								$rate->setPrice($rate->getPrice() + $total_shipping_rate);
							
							}
					}
				}else{
				
					if($freeshippingflag==1){
							foreach($this->_resultFreeShipping->getAllRates() as $rate_free) {
								if(($rate_free->getMethod()==$rate->getMethod()) && $rate->getMethod() == 1){
									$rate->setPrice($rate->getPrice() - $rate_free->getPrice() + $total_shipping_rate);		
								}
							}
        ...
        $request->setPackageWeight($total_package_weight_to_reset_request);
//exit;
        /*$this->setRequest($request);
...
//=========================================================================
//added on 12th September 2014
	if ($request->getResidenceDelivery()) {
        	$destType = $request->getResidenceDelivery();
        } else {
        	$destType = (bool)$this->getConfigData('residence_delivery');
        }
        	//echo "==".$_POST['billing']['testtype']; exit;
        if(isset($_POST['billing']['testtype'])) {
        	$destType = $_POST['billing']['testtype'];
        } // set residential delivery using billing info on one page
        if(isset($_POST['shipping']['testtype'])){
        	$destType = $_POST['shipping']['testtype'];
        }//set residentail delivery using shipping info on one page
       

        $r->setResidenceDelivery((bool)$this->getCode('dest_type', $destType));
//echo "s===".$r->getResidenceDelivery(); exit;
            ...
            'RequestedShipment' => array(
                ...
                'Recipient' => array(
                    'Address' => array(
                        ...
                        'Residential' => $r->getResidenceDelivery()
                    )
                ),
...
//echo "dfdf==";print_r($ratesRequest); exit;
//add the getResidenceDelivery() on 12th September 2014
        return $ratesRequest;
    }
	...
	$allowedMethods = explode(",", $this->getAllowedShippingMethods()); // added on 11
