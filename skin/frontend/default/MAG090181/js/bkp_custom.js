...
$k(document).ready(function($k){ 
	...
	{		
		 $k(".tm_top_currency").hover(function(){
			  $k(".tm_top_currency .sbOptions").slideToggle('slow');
		  });
		 
		 $k(".form-language").hover(function(){
			  $k(".form-language .sbOptions").slideToggle('slow');
		 }); 
		 
	} 
	 
	 $k(".header-cart").click(function () {
		$k("#panel").slideToggle(500);
	});  
	 ...
	 $k(".search-toggle").click(function() { 										 
		 $k('html').click(function() {
				$k(this).unbind('click');  
				$k('.form-search').toggle();			
		   }); 
		$k('.form-search').toggle(); 
		$k(this).next().children("input#search").focus();   
		return false; 
	}); 
...
if ($k(window).width() > 767){	 
	$k("document").ready(function($k){    
		var nav = $k('.nav-container');    
		$k(window).scroll(function () {
			if ($k(this).scrollTop() > 140) {
				nav.addClass("f-nav");
				$k(".header-cart").addClass('fixed');
				$k(".header-search").addClass('fixed');	
			} else {
				nav.removeClass("f-nav");
				$k(".header-cart").removeClass('fixed');
				$k(".header-search").removeClass('fixed');	
			}
		});  
	...
	//$k(window).scroll(function() {
		//$k('.animated').each(function(){
		//var imagePos = $k(this).offset().top;

		//var topOfWindow = $k(window).scrollTop();
			//if (imagePos < topOfWindow+500) {
				//$k(this).addClass("fadeInUp");
			//}
			//else
			//{
			//$k(this).removeClass("fadeInUp");	
			//}
		//});
	//});