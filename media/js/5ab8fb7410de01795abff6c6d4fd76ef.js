	...
	if(navigator.userAgent.indexOf('IE 8')!=-1){
         function replacingClass () {
		   ...
		   if (jQuery(document.body).width() > 479 && jQuery(document.body).width() < 768) { //iPhone
			...
			jQuery('.products-grid.two-products li.item:nth-child(2n+1), .products-grid.three-products li.item:nth-child(3n+1), .products-grid.four-products li.item:nth-child(4n+1), .products-grid.five-products li.item:nth-child(5n+1), .products-grid.six-products li.item:nth-child(6n+1), .products-grid.seven-products li.item:nth-child(7n+1), .products-grid.eight-products li.item:nth-child(8n+1)').css('clear', 'left');
			jQuery('.products-grid.two-products li.item:nth-child(2n), .products-grid.three-products li.item:nth-child(3n), .products-grid.four-products li.item:nth-child(4n), .products-grid.five-products li.item:nth-child(5n), .products-grid.six-products li.item:nth-child(6n), .products-grid.seven-products li.item:nth-child(7n), .products-grid.eight-products li.item:nth-child(8n)').css('clear', 'right');
		}
		replacingClass();
		jQuery(window).resize(function(){replacingClass();});
		jQuery('aside.sidebar > div:last, .products-list li.item:last, .catalog-product-view aside > div:last, #login-holder form .form-list li.fields:last, #login-holder form .form-list li.fields:last .field').addClass('last');
		if(jQuery('#footer').hasClass('original')){
			setTimeout (function(){
				jQuery('.footer-links').each(function(){
					links = jQuery(this);
					footerWidth = jQuery('#footer').width();
					footerCWidth = jQuery('#footer .grid_12').width();
					footerIndent = (footerWidth - footerCWidth)/2;
					if(jQuery('body').hasClass('boxed-layout')) {
						...
						links.css({
							'margin-left' : -footerIndent + 'px',
							'margin-right' : -footerIndent + 'px',
							'padding-left' : footerIndent + 'px',
							'padding-right' : footerIndent + 'px'
						})
					}
				});
			}, 100);
